...
public class MonthlyChargeController {
                ...
                if (currentAuthorService.isCurrentUserAdmin()) {
                    return ResponseEntity.status(HttpStatus.FORBIDDEN)
                            .body(new MessageResponse(
                                    "Admins devem usar endpoints de admin para visualizar cobranças"));
                }
                return ResponseEntity.status(HttpStatus.FORBIDDEN)
    ...
    public ResponseEntity<?> obterPixCode(
        ...
        try {
            ...
            return ResponseEntity
                    .ok(new PixCodeResponse(charge.getPixCode(), charge.getPixImageUrl(), charge.getAmount()));
    ...
    public ResponseEntity<?> confirmarPagamento(
        ...
        try {
            ...
            userRepository.findAllByAuthorId(charge.getAuthorId()).stream()
                    .findFirst()
                    .ifPresent(author -> {
                        ChargeEmailEvent event = new ChargeEmailEvent(
                                ...
                                charge.getDueDate() != null ? charge.getDueDate().toString() : null,
                                null, null,
                                charge.getConfirmedAt() != null
                                        ? java.time.OffsetDateTime.ofInstant(charge.getConfirmedAt(), java.time.ZoneOffset.UTC).toString()
                                        : null,
                                null, null
                        );
                        eventPublisher.publishChargeEvent("billing.payment.confirmed", event);
                    });
    ...
    private MonthlyChargeDTO toDTO(MonthlyCharge charge, String authorId) {
        ...
        return new MonthlyChargeDTO(
                ...
                charge.getPaidAt() != null ? OffsetDateTime.ofInstant(charge.getPaidAt(), java.time.ZoneOffset.UTC)
                        : null,
                confirmedByAdminName,
                charge.getConfirmedAt() != null
                        ? OffsetDateTime.ofInstant(charge.getConfirmedAt(), java.time.ZoneOffset.UTC)
                        : null,
                charge.getPixCode(),
                charge.getPixImageUrl(),
                charge.getPixExpiresAt() != null
                        ? OffsetDateTime.ofInstant(charge.getPixExpiresAt(), java.time.ZoneOffset.UTC)
                        : null,
                daysOverdue,
                hasOpenTicket);
    }
    ...
    public record PixCodeResponse(String pixCode, String pixImageUrl, java.math.BigDecimal amount) {
    }
}
