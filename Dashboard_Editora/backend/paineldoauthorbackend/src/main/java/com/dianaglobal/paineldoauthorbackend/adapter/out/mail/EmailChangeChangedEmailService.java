// src/main/java/com/dianaglobal/paineldoauthor/adapter/out/mail/EmailChangeChangedEmailService.java
...
public class EmailChangeChangedEmailService {
    ...
    /** Aviso de concluído (envia para o NOVO e-mail). */
    public void sendChanged(String toEmail, String name) {
        final String subject = "Seu e-mail foi alterado";
        final String html = buildHtml(name, subject);
    ...
    private String buildHtml(String name, String pageTitle) {
        final String brand = branding.brandName();
        final String safeName = (name == null || name.isBlank()) ? "você" : escapeHtml(name);
        final String safeLogo = normalizeLogoUrl(branding.safeLogoUrl());
        final String loginUrl = branding.frontendUrl() + "/login";
        ...
        return """
                    ...
                    <table role="presentation" width="640" cellspacing="0" cellpadding="0" border="0" style="max-width:640px;background:#ffffff;border:1px solid #e5e7eb;border-radius:12px;overflow:hidden;">
                      ...
                      <tr>
                        <td style="padding:24px;">
                          <p style="font-size:16px;line-height:1.5;margin:0 0 12px;">Olá, <strong>%s</strong>!</p>
                          <p style="font-size:14px;line-height:1.6;margin:0 0 16px;">
                            O e-mail da sua conta foi atualizado com sucesso. Agora você pode fazer login usando o novo e-mail.
                          </p>
                          %s
                        </td>
                      </tr>
            ...
            """.formatted(
                pageTitle, brand,
                header("E-mail atualizado", safeLogo, brand),
                safeName,
                ctaButton(loginUrl, "Ir para login"),
                footer()
        );
    }
    ...
    private String header(String subtitle, String logoUrl, String brand) {
        return """
            ...
            """.formatted(
                imgBoiler(logoUrl, brand, 64, 64, true), // 64x64 para melhor presença + sizing fixo
                escapeHtml(brand),
                escapeHtml(subtitle)
        );
    }
    ...
    /** Garante URL absoluta e HTTPS para reduzir bloqueios em clientes de e-mail. */
